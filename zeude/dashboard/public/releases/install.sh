...
# Configuration
DOWNLOAD_BASE="${ZEUDE_DOWNLOAD_BASE:-http://localhost:3000}"
...
# 5b. Optional companion shims (opencode, copilot)
for COMPANION in opencode copilot; do
    if command -v "$COMPANION" >/dev/null 2>&1; then
        echo -n "Downloading $COMPANION shim... "
        COMP_URL="$DOWNLOAD_BASE/releases/$COMPANION-$PLATFORM"
        if curl -fsSL "$COMP_URL" -o "$INSTALL_DIR/$COMPANION" 2>/dev/null; then
...
2. **Get Dashboard URL**
   ...
   - Look for `dashboard_url=` line, default to `http://localhost:3000`
